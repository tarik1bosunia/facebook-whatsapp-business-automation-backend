...
# Simple JWT Usage Guide

## 1. Obtain Access and Refresh Tokens

To verify that Simple JWT is working, you can issue a POST request to obtain an access and refresh token using valid user credentials:
...
Use the returned access token to authenticate and access protected views:
...
When the short-lived access token expires, use the refresh token to obtain a new one:
...
```json
{
  "access": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyX3BrIjoxLCJ0b2tlbl90eXBlIjoiYWNjZXNzIiwiY29sZF9zdH..."
}
```
...
| `/api/some-protected-view/` | GET    | Access protected resource      |
